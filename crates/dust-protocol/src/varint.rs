...
//!
//! This is the one implementation of the format in this workspace. `dust-net`
//! wrote it first, and when the crates came together its rule was adopted as
//! the rule everywhere: the code lives here because every field codec in this
//! crate reads a VarInt before it reads anything else, and nothing in the
//! workspace may depend on `dust-net`, so the shared thing must sit below both.
//! `dust-net` re-exports this module, so its callers see no change; there is no
//! second decoder anywhere to drift away from it.
//!
//! # The encoding
//!
...
//! transcribed from the protocol's published table rather than produced by
//! the code below them, and the vectors in [`crate::conformance`], computed by
//! implementations this repository does not contain.
//!
...
//! does — the vanilla writer is canonical by construction, and the fuzzers in
//! `dust-net` hold this decoder to exactly the rule it enforces, so the two
//! halves cannot drift apart quietly. A hostile client can still send any
//! *canonical* nonsense it likes; canonicity is a statement about encoding,
//! not about meaning, and nothing here says a decoded number is a sensible
//! one. That is the frame layer's job.
...
mod tests {
    ...
    /// `0x01`.
    ///
    /// The second, independent check is the property sweep in
    /// the fuzzers in `dust-net`: whatever a byte string decodes to must
    /// re-encode to itself, a rule that holds under any convention only when
    /// both halves share the same mistake.
