...
//!
//! Three crates are going to implement pieces of this format independently.
//! `dust-net` will own the VarInt; `dust-nbt` will own NBT. When they land,
//! there will be two readers of one format in one workspace, and **two readers
//! of one format disagree** — not always, not obviously, and usually about the
