...
//! the code below them, and the fixture bytes in [`crate::testkeys`],
//! generated by implementations this repository does not contain.
//!
...
//! does — the vanilla writer is canonical by construction, and the property
//! sweep in `tests/frame_fuzzer.rs` holds this decoder to exactly the rule it
//! enforces, so the two halves cannot drift apart quietly. A hostile client can still send any *canonical* nonsense it
...
mod tests {
    ...
    /// `tests/frame_fuzzer.rs`: whatever a byte string decodes to must
