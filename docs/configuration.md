...
| `view_distance` | `u32` | `8` | restart | How many columns out from a player are streamed, in every direction. A view distance of 8 sends 289 columns on join and 2 sends 25, and the join sends them in one burst: measured against a real world in release, 25 columns take about half a second of streaming and 289 about two, so the difference is roughly five milliseconds a column. The client asks for a distance of its own and is served the smaller of the two, so this is a ceiling rather than a demand. |
